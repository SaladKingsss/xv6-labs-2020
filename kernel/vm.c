...
//什么种类的虚拟地址需要lazy allocation
//如果进程在比 sbrk() 分配的任何内存地址高的虚拟内存地址上出现页面错误，则终止该进程。
//Handle faults on the invalid page below the user stack.   这里到底应该改什么？
uint64 uvmcheck(uint64 va)
{
  pte_t *pte;
...
void uvmlazyalloc(uint64 va)
{
  ...
  {
    memset((void *)ka, 0, PGSIZE); //初始化为0（实际是复制字符）
    va = PGROUNDDOWN(va);
